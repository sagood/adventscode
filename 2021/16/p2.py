...
class Packet:
    ...
    def eval(self):
        if self.type_id == 0:
            return sum(item.eval() for item in self.sub_packets)
        elif self.type_id == 1:
            return math.prod(item.eval() for item in self.sub_packets)
        elif self.type_id == 2:
            return min(item.eval() for item in self.sub_packets)
        elif self.type_id == 3:
            return max(item.eval() for item in self.sub_packets)
        elif self.type_id == 4:
            return self.value
        elif self.type_id == 5:
